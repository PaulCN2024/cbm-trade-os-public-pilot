...
const { sanitizeCompanyInput, safetyBoundaryPayload } = require("../lib/api-validation.js");
...
module.exports = async function handler(request, response) {
  try {
    ...
    if (request.method === "GET") {
      ...
      sendJson(response, 200, id ? { company: data } : { companies: data || [] });
      return;
    }

    if (request.method === "POST") {
      const input = sanitizeCompanyInput(parseBody(request).company || parseBody(request));
      if (!input.company_name) {
        sendJson(response, 400, { error: "company_name is required." });
        return;
      }
      const { data, error } = await supabase.from("companies").insert(input).select("*").single();
      if (error) throw error;
      sendJson(response, 201, { company: data, ...safetyBoundaryPayload() });
      return;
    }
    ...
    if (request.method === "PATCH" || request.method === "PUT") {
      ...
      const input = sanitizeCompanyInput(body.company || body);
      const { data, error } = await supabase.from("companies").update(input).eq("id", id).select("*").single();
      if (error) throw error;
      sendJson(response, 200, { company: data, ...safetyBoundaryPayload() });
      return;
    }
